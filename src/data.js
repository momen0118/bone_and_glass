...
// 細工師(クリア後限定の新客・宝飾職人の女性)。石(鉱物)だけを見に来て最高の一点を高値で買う。
// 値付けの駆け引きが通じない(表示価格×1.3で買う)。蟲屋・大家に続く「よそから来る専門家」の三人目。
export const SAIKUSHI = {
  id: "saikushi", name: "細工師", icon: "💎",
  mult: 1.3, // 支払い倍率(基準価×これ。値付け倍率の影響を受けない・職人価格)
  // 初回来店の三段構成(タップ送り)。以降は通常の夜カード
  first: [
    "失礼。……あら、いい石。誰が磨いたの? ——あなた? ふうん",
    "私は細工師。石を探して店を回ってる。——ええ、女の。驚いた顔は見飽きたわ",
    "悪くない店よ。石の置き方は、なってないけど",
  ],
  buy: [
    "これ。……磨きは及第点。石が良いから買うのよ",
    "あなたにしては、いい仕事",
    "もらうわ。値切らない——石に失礼だから",
  ],
  pass: [
    "今日は目当てがないわね。……石の、話よ?",
    "この並べ方で売れてるの? ……売れてるなら、いいけど",
  ],
  big: ["……これは。……ふん、悔しいけど、文句がないわ"],
};
// 細工師の出現条件(クリア後・棚に鉱物がこの点数以上)。来客抽選に weight 1 で混ざる
export const SAIKUSHI_MINERALS = 3;
...
export const CUSTOMERS = [
  { id: "gakusei", name: "学生", icon: "📓", minRep: 0, budget: 150, weight: 4, easyBuyer: true,
    likesTags: [], likesCats: [],
    lines: {
      buy: ["小遣いで買えるのを探してて…", "レポートの資料にしたいんです", "ここ、落ち着きますね", "授業で使うんです、これ"],
      regular: ["また来ちゃいました", "先生に自慢したら羨ましがられました"],
      friend: ["この店で買った頭骨、下宿の机に置いてます"],
      pass: ["……今月は下宿代が。また来ます", "うう、悩む。悩んで、帰ります"],
      poor: ["桁がひとつ違うや……見てるだけでも楽しいけど", "今月は、これが精一杯"],
      big: ["一世一代の買い物です!"],
    } },
  ...
  { id: "wakate", name: "若い研究者", icon: "🎓", minRep: 0, budget: 420, weight: 2, flag: "gakuseiGraduated",
    ...
    lines: {
      buy: ["研究室の棚に、この店の品を", "給料日なので"],
      regular: ["後輩たちにも教えてます、この店"],
      friend: ["あの日の約束から、全部始まった気がします"],
      pass: ["論文の締切が……また来ます"],
...
// 開店の夜(新規1日目・最初の客の学生。一度きり・タップ送り・入れ替え式)
export const OPENING_NIGHT = [
  "こんなところに、標本の店……。",
  "すみません、見てるだけで。……あの、これ、全部本物ですか。",
  "……就職したら。就職したら、大きいのを買いに来ます。",
];
// 好事家の九枠買い(隠しイベント・タップ送り)
export const NINE_BUY = ["————。", "全部だ。全部、包んでくれ。"];

// 就職イベント後、学生(後輩)の buy に加わる一言
export const GAKUSEI_KOUHAI_LINE = "先輩に教わったんです、この店";
// 学生の就職イベント(棚に商品が1点以上ある夜に発生。最も高い品を予算無視で購入)
// 就職の夜。話者・肖像は「若い研究者」(wakate)。三行を入れ替え式タップ送りで見せ、
// 最終行のあとにト書き(sub)→購入カードへ繋ぐ。正史: 約束の学生は就職の夜まで一度も店に来ていない。
export const GAKUSEI_GRAD = {
  threshold: 25, // 学生への累計販売がこの回数に達した後、次の来店で発生
  line: "……やっと、来れました",
  line2: "就職したんです。だから——約束を、果たしに",
  line3: "一番いいものを、ください",
...
export const SCENES = {
  ...
  endingNight: { cid: "collector", portraitFrom: 1, seq: [
    ...
    { t: "narr", img: { kind: "op", id: "kaimen" }, text: "布包みが帳場に置かれた。解くと、硝子のような細工物——いや、骨だ。硝子の骨だ。" },
    { t: "line", text: "深海の海綿だ。若い頃に手に入れて、仕立てられる職人を、ずっと探していた。" },
    { t: "line", text: "……お前なら、できるだろう。" },
    { t: "narr", text: "返事を待たずに、扉は閉まった。月の明るい晩だった。" },
  ] },
  // v7. 硝子の花籠 完成の一幕(肖像なし・画像+地の文)
  hanakagoDone: { cid: null, seq: [
    { t: "narr", img: { kind: "spec", id: "s_hanakago" }, text: "硝子の棚の、いちばん高いところに飾った。——売り物ではない。" },
  ] },
};
...
export const LEDGER_LOST = "——ここまでの記録は、失われている。";
// 客層称号: 通算の最多客層に応じて集計欄の末尾に一行(若い研究者は学生に合算。最多が入れ替われば変わる)
export const LEDGER_TITLES = {
  gakusei: "——学生たちの店、と呼ぶ者もいる。",
  gakusha: "——研究室の隣、と呼ぶ者もいる。",
  koujika: "——蒐集家泣かせ、と呼ぶ者もいる。",
  kifujin: "——客間の誉れ、と呼ぶ者もいる。",
};
